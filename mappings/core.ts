...
export function handleSync(event: Sync): void {
  ...
  blueberry.totalLiquidityAVAX = blueberry.totalLiquidityAVAX.minus(
    pair.trackedReserveAVAX as BigDecimal
  );
  ...
  blueberry.totalLiquidityAVAX =
    blueberry.totalLiquidityAVAX.plus(trackedLiquidityAVAX);
  blueberry.totalLiquidityUSD = blueberry.totalLiquidityAVAX.times(
    bundle.avaxPrice
  );
...
export function handleMint(event: Mint): void {
  ...
  blueberry.totalTransactions = blueberry.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let blueberry = BlueberryFactory.load(FACTORY_ADDRESS);
  blueberry.totalVolumeUSD = blueberry.totalVolumeUSD.plus(trackedAmountUSD);
  blueberry.totalVolumeAVAX = blueberry.totalVolumeAVAX.plus(trackedAmountAVAX);
  blueberry.untrackedVolumeUSD =
    blueberry.untrackedVolumeUSD.plus(derivedAmountUSD);
  blueberry.totalTransactions = blueberry.totalTransactions.plus(ONE_BI);
  ...
  let blueberryDayData = updateBlueberryDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  blueberryDayData.dailyVolumeUSD =
    blueberryDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  blueberryDayData.dailyVolumeAVAX =
    blueberryDayData.dailyVolumeAVAX.plus(trackedAmountAVAX);
  blueberryDayData.dailyVolumeUntracked =
    blueberryDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  ...
  token0DayData.dailyVolumeAVAX = token0DayData.dailyVolumeAVAX.plus(
    amount0Total.times(token0.derivedAVAX as BigDecimal)
  );
  ...
  token1DayData.dailyVolumeAVAX = token1DayData.dailyVolumeAVAX.plus(
    amount1Total.times(token1.derivedAVAX as BigDecimal)
  );
